...
*** VILLAINS
    ...
    + Farmer Blue :: From Marty the Mole
** WORLD MAP
...
** CARD DESCRIPTIONS
   ...
   + The following words are capitalized: Minion, Effect (in the
     context of an Effect card), Hero, Cost, Level, Morale, Phase (and
     all of the phase names).
...
*** DEFINITIONS
    ...
    + Play :: A card is "played" when it moves from a position not on
      ...
      normal turn, it costs EP to do so.
    + Summon :: This is an outdated synonym for "Play". You may see it
      in some old notes for this game, but card effects should NOT use
      this word. Prefer the word "play" instead.
...
*** INFLUENCE CHECKS
    ...
    whenever a card of any kind attempts to change another card.

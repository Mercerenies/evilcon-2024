...
func get_text() -> String:
    return "+1 Level to your most powerful Minion, or +1 Level to all of your Minions if you successfully played Squaredude this turn."
...
func ai_get_score(playing_field, player: StringName, priorities) -> float:
    ...
    if playing_field.event_logger.has_event(playing_field.turn_number, player, LogEvents.SQUAREDUDE_PLAYED):
        ...
        # We haven't played Squaredude yet. First, consider the value
        # of playing Circlegirl alone.
        ...
        # Second, if playing Squaredude immediately after would have
        # positive value, then consider that too.
        if Query.on(playing_field).hand(player).any(Query.by_id(PlayingCardCodex.ID.SQUAREDUDE)):
